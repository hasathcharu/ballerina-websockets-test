...
isolated service class WsServiceUser {
    *websocket:Service;
    ...
    isolated remote function onSubscribe(websocket:Caller caller, types:Subscribe sub) returns stream<types:Response,error> {
        io:println("Subscribe: " + caller.getConnectionId());
        UserChat user = new ({caller: caller, gender: sub.gender, name: sub.name, id: caller.getConnectionId()});
        lock {
	        users[caller.getConnectionId()] = user;
        }
        user.addChat({message: "Welcome to the chat " + sub.name, event: "chat", toUserId: caller.getConnectionId()});
        return new (new ChatStream(user));
    }
    ...
    isolated remote function onChat(websocket:Caller caller, types:Chat message) returns error? {
        ...
        UserChat toUser;
        lock {
            // fromUser = users.get(caller.getConnectionId());
            toUser = users.get(message.toUserId);
        }
        types:Chat fromChat = message.clone();
        fromChat.message = "You: " + message.message;
        types:Chat toChat = message.clone();
        toChat.message = "TEST" + ": " + message.message;
