...
public function main() returns error? {
    worker subscribe returns error? {
        ...
        stream<Response,error?> subscription = check chatClient->doSubscribe({"event":"subscribe","name":"Ballerina","gender":"Female", "id":"NULL"}, 10);
        while true {
    ...
    Response|error response = chatClient->doChat({"event":"chat","message":message, "toUserId": toUser, "id":"NULL"}, 10);
